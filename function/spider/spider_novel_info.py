...
def save_to_MYSQL(datas):
    # 连接到数据库
    db = pymysql.connect(host='10.25.119.1', user='txu', password='password', port=3306, db='spiders')
    cursor = db.cursor()
    ...
    for data in datas:
        try:
            ...
            cursor.execute(sql, (
            data['novel_name'], data['author'], data['read_num'], data['novel_type'], data['status'], data['id']))
            db.commit()
